...
def telegram_bot_sendtext(bot_message, bot_chatID):
    global bot_token
    params = {'chat_id': bot_chatID, 'text': bot_message}
...
schedule.every().day.at("09:12").do(report)

def main():
    update_id = last_update(get_updates_json(url))['update_id']
    while True:
        if update_id == last_update(get_updates_json(url))['update_id']:
           telegram_bot_sendtext("Greetings! I'm Mornin' From Callie, a bot which wishes you good morning at 6.30 am, and hopefully wakes you up! Well, it's a good habit to rise and shine early anyway.", str(get_chat_id(last_update(get_updates_json(url)))))
           update_id += 1
           if str(get_chat_id(last_update(get_updates_json(url)))) not in bot_chatIDs:
               bot_chatID.append(str(get_chat_id(last_update(get_updates_json(url)))))
